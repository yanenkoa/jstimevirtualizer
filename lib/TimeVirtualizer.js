...
TimeVirtualizer.prototype._advanceTimeMSInSafeContext = function(durationMS) {
    ...
    while(durationMS > 0) {
        ...
        // is the timeout with lowest fireTSMS.
        if(isUndefOrNull(timeout)) {
            this._virtTSMS += durationMS;
            break;
        }
        var proceedTime = Math.min(timeout.fireTSMS - this._virtTSMS, durationMS);
        ...
        if (timeout.fireTSMS <= this._virtTSMS && timeout.isValid()) {
            ...
            if (!timeout.getIsPeriodic()) {
                // In case this is not an interval, delete it
                this._timeouts.splice(0,1);
            } else {
                // Otherwise alter fireTSMS accordingly and sort this._timeouts
                timeout.fireTSMS += timeout.getDelayMS();

                var i = 0;
                while(i+1 < this._timeouts.length && this._timeouts[i].fireTSMS > this._timeouts[i+1].fireTSMS) {
                    var tmp = this._timeouts[i];
                    this._timeouts[i] = this._timeouts[i+1];
                    this._timeouts[i+1] = tmp;
                    i++;
                }
            }
        }
...
TimeVirtualizer.prototype._setIntervalOrTimeout = function(func, delayMS, isPeriodic, funcParams) {
    ...
    var realClearFunc;
    if (isPeriodic) {
        realClearFunc = this._clearInterval;
    }
    else {
        realClearFunc = this._clearTimeout;
    }

    var timeout = new Timeout(
        realFunc, realClearFunc,
    ...
    // this._timeouts array needs to be sorted by fireTSMS
    var i = this._timeouts.length - 1;
    // inserting new timeout into the right place
    while(i > 0 && this._timeouts[i].fireTSMS < this._timeouts[i-1].fireTSMS) {
        var tmp = this._timeouts[i];
        this._timeouts[i] = this._timeouts[i-1];
        this._timeouts[i-1] = tmp;
        i--;
