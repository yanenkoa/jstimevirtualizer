...
function TimeVirtualizer() {
    ...
    this._intervalForAnimFrames = 0;
    this._lastAnimFrameFire = 0;
...
TimeVirtualizer.prototype.setFPS = function(FPS) {
    if (FPS == 0) {
        ...
        this._intervalForAnimFrames = Math.floor(1000/FPS);
        this._lastAnimFrameFire = timeVirtualizer._virtDate.now();
    };
};
...
TimeVirtualizer.prototype._advanceTimeMSInSafeContext = function(durationMS) {
    ...
    while(durationMS > 0) {
        ...
        var nextAnimFrameFire = this._lastAnimFrameFire + this._intervalForAnimFrames;

        var proceedTime;
        if (isUndefOrNull(timeout)) {
            if (this._intervalForAnimFrames == 0) {
                proceedTime = durationMS;
            } else {
                proceedTime = Math.min(durationMS, nextAnimFrameFire - this._virtTSMS);
            }
        } else {
            if (this._intervalForAnimFrames == 0) {
        ...
        if (this._virtTSMS == nextAnimFrameFire) {
            // Extra protection from anim frames callbacks side effects needed
            ...
            this._lastAnimFrameFire = this._virtTSMS;
        }
    }
};
